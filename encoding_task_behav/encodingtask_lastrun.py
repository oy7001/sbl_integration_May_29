...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/oliviayin/Desktop/sbl_realistic integration/encoding_task_behav/encodingtask_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    df = pd.read_csv('counterbalanced_vars_behav.csv', header=1)
    
    # clean columns HARD
    df.columns = df.columns.str.strip()
    
    # force numeric (THIS is key)
    df['enc_relatedness_keys'] = pd.to_numeric(df['enc_relatedness_keys'], errors='coerce')
    ...
    print("enc_relatedness_keys value:", df.loc[ind, 'enc_relatedness_keys'])
