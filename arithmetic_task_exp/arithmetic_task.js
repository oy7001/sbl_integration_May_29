...
function instruction_screenRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instruction_screenComponents)
...
function loop_restLoopBegin(loop_restLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisLoop_rest of loop_rest) {
...
function set_optionsRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of set_optionsComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function is_restRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of is_restComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function rest_screenRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of rest_screenComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
